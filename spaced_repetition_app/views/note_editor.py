...
class NoteEditorWindow(tk.Toplevel):
    ...
    def _build_filters(self) -> None:
        frame = ttk.LabelFrame(self, text="Фильтры")
        frame.pack(fill="x", padx=10, pady=10)

        ttk.Label(frame, text="Колода:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.deck_combo = ttk.Combobox(frame, textvariable=self.deck_var, values=list(self.deck_map.keys()), state="readonly")
        ...
        ttk.Label(frame, text="Поиск:").grid(row=0, column=2, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(frame, text="Теги (через запятую):").grid(row=0, column=4, padx=5, pady=5, sticky="w")
        ...
        ttk.Button(frame, text="Применить", command=self.refresh_notes).grid(row=0, column=6, padx=5, pady=5)
    ...
    def _build_buttons(self) -> None:
        ...
        ttk.Button(frame, text="Добавить", command=self.add_note).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame, text="Редактировать", command=self.edit_note).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame, text="Удалить", command=self.delete_note).pack(side=tk.LEFT, padx=5)
...
class NoteForm(tk.Toplevel):
    ...
    def _build_form(self) -> None:
        ...
        ttk.Label(frame, text="Колода:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        combo = ttk.Combobox(frame, textvariable=self.deck_var, values=[d["name"] for d in self.decks], state="readonly")
        combo.grid(row=0, column=1, sticky="ew", padx=5, pady=5)

        ttk.Label(frame, text="Front:").grid(row=1, column=0, sticky="nw", padx=5, pady=5)
        self.front_text = tk.Text(frame, height=6)
        self.front_text.grid(row=1, column=1, sticky="ew", padx=5, pady=5)

        ttk.Label(frame, text="Back:").grid(row=2, column=0, sticky="nw", padx=5, pady=5)
        self.back_text = tk.Text(frame, height=6)
        self.back_text.grid(row=2, column=1, sticky="ew", padx=5, pady=5)

        ttk.Label(frame, text="Теги:").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(frame, textvariable=self.tags_var).grid(row=3, column=1, sticky="ew", padx=5, pady=5)

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=4, column=1, sticky="e", padx=5, pady=10)
