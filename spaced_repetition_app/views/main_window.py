...
class MainWindow(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, text=f"Пользователь: {self.user['email']}", font=("TkDefaultFont", 12, "bold")).pack(
            side=tk.LEFT
        )

        ttk.Button(header, text="Обновить", command=self.refresh_data).pack(side=tk.RIGHT)
        ...
        for idx, (key, label) in enumerate(
            ...
            ttk.Label(frame, textvariable=self.stats_vars[key], font=("TkDefaultFont", 14, "bold")).pack()

        decks_frame = ttk.LabelFrame(self, text="Колоды")
        decks_frame.pack(fill="both", expand=True, padx=10, pady=10)
        ...
        self.deck_tree.heading("due", text="Due")
        ...
        scrollbar = ttk.Scrollbar(decks_frame, orient="vertical", command=self.deck_tree.yview)
        ...
        ttk.Button(buttons_frame, text="Менеджер колод", command=self.open_deck_manager).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(buttons_frame, text="Редактор карточек", command=self.open_note_editor).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(buttons_frame, text="Сессия повторения", command=self.open_review_session).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(buttons_frame, text="Прогресс", command=self.open_progress_window).pack(
            side=tk.LEFT, padx=5
        )

        status_bar = ttk.Frame(self)
        status_bar.pack(fill="x", side=tk.BOTTOM, padx=10, pady=5)
        ttk.Label(status_bar, textvariable=self.stats_vars["time"], anchor="e").pack(side=tk.RIGHT)
