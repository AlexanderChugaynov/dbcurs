...
class ReviewSessionWindow(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        header = ttk.Frame(container, style="Toolbar.TFrame")
        header.pack(fill="x", pady=(0, 10))
        deck_name = "Все колоды" if not self.deck_id else self._deck_name()
        ttk.Label(header, text=f"Колода: {deck_name}", style="Subtitle.TLabel").pack(anchor="w")

        card_frame = ttk.Frame(container, style="Flashcard.TFrame")
        card_frame.pack(fill="both", expand=True)

        self.front_label = ttk.Label(card_frame, text="", style="FlashcardFront.TLabel", justify="center")
        self.front_label.pack(fill="x", pady=(0, 10))

        self.back_label = ttk.Label(card_frame, text="", style="FlashcardBack.TLabel", justify="center")
        self.back_label.pack(fill="x")

        controls = ttk.Frame(container, style="Toolbar.TFrame")
        controls.pack(fill="x", pady=(12, 0))

        ttk.Button(controls, text="Показать ответ", command=self.show_answer, style="Accent.TButton").pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(controls, text="Пропустить", command=self.skip_card, style="Secondary.TButton").pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(controls, text="Пауза карточки", command=self.suspend_card, style="Secondary.TButton").pack(
            side=tk.LEFT, padx=5
        )

        quality_frame = ttk.LabelFrame(
            container,
            text="Оценка качества (0-5)",
        ...
        for quality in range(6):
            ttk.Button(
                ...
                style="Secondary.TButton" if quality < 3 else "Accent.TButton",
            ).pack(side=tk.LEFT, expand=True, fill="x", padx=6, pady=6)

        status_bar = ttk.Frame(container, style="StatusBar.TFrame", padding=8)
